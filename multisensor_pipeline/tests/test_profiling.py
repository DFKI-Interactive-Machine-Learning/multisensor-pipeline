...
class ProfilingTest(unittest.TestCase):
    def _test_frequency_stats(self):
        ...
        self.assertLessEqual(stats["test"]._cma, 20) and \
            self.assertGreater(stats["test"]._cma, 19)
        self.assertLessEqual(stats["test"]._sma, 20) and \
            self.assertGreater(stats["test"]._sma, 19)
